...
const generateAndStoreWords = async (): Promise<void> => {
  try {
...
`
    ...
    let wordsArray;
    try {
      wordsArray = JSON.parse(content) as Array<{
        text: string;
        meaningHindi: string;
...
// API Endpoints
app.get("/", (c: Context) => c.text("English Vocabulary API"));

app.get("/words", async (c: Context) => {
...
app.post("/generate-words", async (c: Context) => {
  await generateAndStoreWords();
  return c.json({ message: "Generated 10+ words." });
});

app.post("/trigger-cron", async (c: Context) => {
  await generateAndStoreWords();
